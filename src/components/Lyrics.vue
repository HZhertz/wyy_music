  ...
  <div>
    <div class="lyrics-main" :style="{ maxHeight: maxH + 'px' }">
      <div class="lyrics" ref="lyrics" v-if="lyricObj.length" :style="transform">
        <p :class="[isCurLyric(index)]" v-for="(item, index) in lyricObj" :key="index">
          {{ item.txt }}
        </p>
      </div>
...
<script setup>
import { computed, getCurrentInstance, onMounted, watch, reactive, toRefs } from 'vue'
...
const { lyric, lyricObj } = toRefs(info)
